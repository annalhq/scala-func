...
class KMeans extends KMeansInterface:
  ...
  def classify(points: Seq[Point], means: Seq[Point]): Map[Point, Seq[Point]] =
    ...
    val meansMap = points.groupBy(p => findClosest(p, means))
    retMap ++ meansMap
  ...
  final def kMeans(points: ParSeq[Point], means: ParSeq[Point], eta: Double): ParSeq[Point] =
    ...
    val deltaMeans = means.zip(newMeans).map((a, b) => a.squareDistance(b))
    val isConverged = converged(eta, means, newMeans)
    if (isConverged) newMeans else kMeans(points, newMeans, eta)
